...
func TestHybridCache_MixedRunStates(t *testing.T) {
	...
	runs := []models.RunResponse{
		{ID: "active-1", Status: models.StatusQueued, CreatedAt: time.Now().Add(-4 * time.Hour)},
		{ID: "active-2", Status: models.StatusProcessing, CreatedAt: time.Now().Add(-3 * time.Hour)},
		{ID: "terminal-1", Status: models.StatusDone, CreatedAt: time.Now().Add(-2 * time.Hour)},
		{ID: "terminal-2", Status: models.StatusFailed, CreatedAt: time.Now().Add(-1 * time.Hour)},
		{ID: "active-3", Status: models.StatusInitializing, CreatedAt: time.Now()},
	}
	...
	assert.Equal(t, "active-3", cachedRuns[0].ID)
	assert.Equal(t, "terminal-2", cachedRuns[1].ID)
...
func TestHybridCache_GetStats(t *testing.T) {
	...
	// Add data
	runs := []models.RunResponse{
		{ID: "active-1", Status: models.StatusProcessing},
		{ID: "active-2", Status: models.StatusQueued},
		{ID: "terminal-1", Status: models.StatusDone},
		{ID: "terminal-2", Status: models.StatusFailed},
	}
