...
// refreshViewportContent refreshes all viewport content
func (d *DashboardView) refreshViewportContent() {
	d.updateViewportContent()
}

// updateColumnWidths updates column width calculations
func (d *DashboardView) updateColumnWidths() {
	if d.width <= 0 {
		return
	}

	// Calculate column widths for various layouts
	switch d.currentLayout {
	case models.LayoutTripleColumn:
		// Already handled in updateViewportSizes
		d.updateViewportSizes()
	case models.LayoutAllRuns:
		// All runs layout uses full width for the list
		if d.allRunsList != nil {
			d.allRunsList.SetSize(d.width-4, d.height-6) // Account for borders and status line
		}
...
// batchUpdateViewports updates multiple viewports efficiently
func (d *DashboardView) batchUpdateViewports(updateRepo, updateRuns, updateDetails bool) {
	if updateRepo {
		d.updateRepoViewportContent()
	}
	if updateRuns {
